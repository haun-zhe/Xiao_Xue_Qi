...
def get_homePage_Data():
    sql_number_oldPeople = text("SELECT COUNT(*) FROM oldperson_info")
    sql_number_gender = text(
        "SELECT COUNT(case when oldperson_info.gender='male' then oldperson_info.gender end) as 'male', " \
        "COUNT(case when oldperson_info.gender='female' then oldperson_info.gender end) as 'female' " \
        "FROM oldperson_info")
    sql_numberOfNew_oldPeople = text(
        "SELECT COUNT(*) FROM oldperson_info WHERE DATE(oldperson_info.checkin_date)=CURDATE()")
    sql_number_volunteer = text("SELECT COUNT(*) FROM volunteer_info")
    sql_numberOfActive_volunteer = text("SELECT COUNT(*) FROM volunteer_info WHERE ISACTIVE='Yes'")
    sql_number_emotion = text(
        "SELECT COUNT(case when event_info.event_desc='anger' then event_info.event_desc end) as 'anger', " \
    ...
    sql_number_fall = text(
        "SELECT COUNT(case when DATE(event_info.event_date) BETWEEN DATE_SUB(CURDATE(),INTERVAL 3 DAY) AND CURDATE() then event_info.event_date end) as 'within_four_days', " \
...
def get_old_person_info():
    try:
        result_list = []
        sql = text('SELECT username, profile_photo, id_card, gender, room_number, checkin_date FROM oldPerson_info')
        for result in db.session.execute(sql):
            username = result[0]
            profile_photo_path = result[1]  # Assuming this is the path to the profile photo
            id_card = str(result[2])
            gender = result[3]
            room_num = result[4]
            checkin_date = result[5].strftime('%Y-%m-%d')  # Format date as string
            ...
            if profile_photo_path and os.path.exists(profile_photo_path):
                ...
                encode_str = ""  # Or you can provide a default image URL or base64 string here
        ...
        response = {
            'code': 200,
            'data': result_list,
            'msg': 'Succeed to select all old persons!'
        }
        ...
        # Rollback the transaction explicitly if needed
        db.session.rollback()
        # Return an error response
        return jsonify({
            'code': 500,
            'msg': 'Failed to fetch old persons data'
        })
...
def get_volunteer():
    try:
        result_list = []
        sql = text("SELECT name, profile_photo, id_card, gender, phone, ISACTIVE FROM volunteer_info")
        for result in db.session.execute(sql):
            name = result[0]
            profile_photo_path = result[1]  # 假设这是头像的路径
            id_card = str(result[2])
            gender = result[3]
            phone = result[4]
            is_active = str(result[5])
            ...
            if profile_photo_path and os.path.exists(profile_photo_path):
                ...
                encode_str = ""  # 或者你可以提供一个默认的图像URL或base64字符串
...
def identify_id_card(photo):
    ...
    key = "KL4qEGp6E4DsWi9MlJjtDNPl9w7fDBx7"
    secret = "lJNUaMxsWaEsUr8aH6aU5jze2ysWwxX5"
    base64 = photo
