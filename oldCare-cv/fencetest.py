...
# 全局变量
prototxt_file_path = './models_saving/mobilenet_ssd/MobileNetSSD_deploy.prototxt'
model_file_path = './models_saving/mobilenet_ssd/MobileNetSSD_deploy.caffemodel'
output_fence_path = './images/invade'
skip_frames = 30
# your python path
python_path = 'D:/softward/python27'
...
def check_fence(frame):
    global prototxt_file_path, model_file_path, output_fence_path, skip_frames,image
    ...
    for (objectID, centroid) in objects.items():
        ...
        if to is None:
            ...
            if not to.counted:
                # 如果质心在禁区内
                if y2 <= centroid[1] <= y3 and x1 <= centroid[0] <= x2:
                    totalDown += 1
                    to.counted = True
                    ...
                    insert(3, event_location, event_desc, image_base, 'snapshot__%s.jpg' % current_time)
...
def fence_main():
    global x1, y1, x2, y2, x3, y3, x4, y4, W, H,totalFrames,image
    # 得到当前时间
    ...
    # url = 'rtsp://admin:123@192.168.43.206:8554/live'
